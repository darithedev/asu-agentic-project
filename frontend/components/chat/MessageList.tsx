            ...
            <div className="flex flex-col gap-2 w-full">
              {exampleQueries.map((query, idx) => (
                <button
                  ...
                  className="w-full text-left text-sm p-3 rounded-lg border border-orange-200/60 dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-gray-700 hover:border-orange-300/70 dark:hover:border-gray-600 transition-all shadow-sm hover:shadow-md text-foreground break-words whitespace-normal"
                >
                  {query.text}
                </button>
              ))}
  ...
  return (
    <ScrollArea className="h-full w-full" ref={scrollAreaRef}>
      <div className="flex flex-col gap-3 sm:gap-4 p-3 sm:p-4">
        {messages
          .filter((message) => {
            // Filter out assistant messages with empty content when not loading
            // This prevents showing empty bubbles for old conversations
            if (message.role === "assistant" && !message.content.trim() && !isLoading) {
              return false;
            }
            return true;
          })
